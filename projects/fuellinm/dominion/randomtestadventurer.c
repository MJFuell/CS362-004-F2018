...
//current player reveals cards in their deck until they're found 2 treasures (copper, silver, gold)
//2 treasures are added to player hand
//rest of the cards revealed are discarded
...
int testAdventurer(int p, struct gameState *post)
{
     ...
     int x;
     int dt = 0;              //drawn treasure count (starts at 0)
     int curPlayer = 1;       //current player identifier
     int cpos = 1;            //card position
     int tempHand[100];

     //printf("calling bug_smithy\n");
     x = bug_adventurer(dt, post, curPlayer, cpos, tempHand);
...
int main()
{
     ...
     printf("Testing bugged adventurer.\n");
